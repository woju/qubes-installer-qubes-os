...
# Only for upgrade purposes, /run will be tmpfs mounted later
if [ ! -d $ROOT/run ]; then
    echo "Preparing /run directory"
    mkdir $ROOT/run
    if [ -d $ROOT/var/run.runmove~ ]; then
        cp -a $ROOT/var/run.runmove~/* $ROOT/run/
    fi
fi

if [ ! -d $ROOT/run/lock ]; then
    echo "Preparing /run/lock directory"
    mkdir $ROOT/run/lock
    if [ -d $ROOT/var/lock.lockmove~ ]; then
        cp -a $ROOT/var/lock.lockmove~/* $ROOT/run/lock/
    fi
fi
